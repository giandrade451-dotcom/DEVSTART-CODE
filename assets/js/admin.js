  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderLogin() {
      root.innerHTML = `
        <div class="auth-wrap" style="min-height:auto; padding-top:60px;">
          <div class="auth-card reveal">
            ...
            <p class="sub">Restricted area. Enter the admin credentials to continue.</p>
            ...
            <div class="auth-alt small">Default: <code>admin</code> / <code>devstart2025</code></div>
          </div>
        </div>
      `;
      ...
      document.getElementById("admin-login").addEventListener("submit", (e) => {
        ...
        try {
          ...
          toast({ title: "Admin signed in", type: "success" });
          renderPanel();
        } catch (err) {
          toast({ title: "Access denied", message: err.message, type: "error" });
        }
      });
    }
    ...
    function renderPanel() {
      ...
      root.innerHTML = `
        <header class="dash-head reveal">
          <div>
            <h1>Admin Panel 🛠️</h1>
            <div class="sub">Manage users, VIP access, and course availability.</div>
          </div>
          <button class="btn danger" id="admin-logout">Sign out</button>
        </header>
        ...
        <section class="card reveal mb-3">
          <div class="row between mb-2" style="flex-wrap:wrap;gap:12px;">
            ...
            <input class="input" id="user-search" placeholder="Search by username…" style="min-width:260px;" />
          </div>
          <div class="table-wrap">
            <table class="table">
              <thead>
                <tr><th>Username</th><th>Plan</th><th>Joined</th><th>Lessons done</th><th>Quizzes</th><th>Actions</th></tr>
              </thead>
        ...
        <section class="card reveal">
          <h2 style="margin-top:0;">Course availability</h2>
          <p class="text-muted">Lock a course to hide it from every user (including VIPs). Useful for temporary maintenance or re-editing.</p>
          <div class="table-wrap">
            <table class="table">
              <thead><tr><th>Course</th><th>Type</th><th>Lessons</th><th>Status</th><th>Actions</th></tr></thead>
      ...
      document.getElementById("admin-logout").addEventListener("click", () => {
        admin.logout();
        toast({ title: "Admin signed out", type: "info" });
        renderLogin();
      });
    ...
    function renderStats() {
      ...
      document.getElementById("admin-stats").innerHTML = [
        { lab: "Total users", num: all.length, sub: "All registered accounts" },
        { lab: "VIP users", num: vipCount, sub: `${all.length ? Math.round((vipCount/all.length)*100) : 0}% of total` },
        { lab: "Lessons completed", num: totalLessons, sub: "Across all users" },
        { lab: "Courses locked", num: lockedCount, sub: `of ${courses.length} total` },
    ...
    function renderUsers(query) {
      ...
      if (!list.length) {
        tbody.innerHTML = `<tr><td colspan="6" class="center text-muted" style="padding:30px;">No users match.</td></tr>`;
        return;
      }
      tbody.innerHTML = list.map(u => {
        ...
        return `<tr>
          ...
          <td>${u.vip ? `<span class="badge vip">VIP</span>` : `<span class="badge">Free</span>`}</td>
          <td>${new Date(u.createdAt).toLocaleDateString()}</td>
          <td>${lessons}</td>
          <td>${quizzes}</td>
          <td>
            <div class="row" style="gap:6px;flex-wrap:wrap;">
              <button class="btn sm ${u.vip ? "danger" : "vip"}" data-toggle-vip="${escapeHtml(u.username)}">${u.vip ? "Revoke VIP" : "Unlock VIP"}</button>
              <button class="btn sm" data-view="${escapeHtml(u.username)}">Details</button>
              <button class="btn sm" data-reset="${escapeHtml(u.username)}">Reset progress</button>
              <button class="btn sm danger" data-delete="${escapeHtml(u.username)}">Delete</button>
            </div>
          </td>
        </tr>`;
      }).join("");
      ...
      tbody.querySelectorAll("[data-toggle-vip]").forEach(btn => btn.addEventListener("click", () => {
        ...
        toast({ title: u.vip ? "VIP revoked" : "VIP unlocked", message: `@${name}`, type: u.vip ? "info" : "success" });
      ...
      tbody.querySelectorAll("[data-reset]").forEach(btn => btn.addEventListener("click", () => {
        ...
        toast({ title: "Progress reset", message: `@${name}`, type: "info" });
        renderUsers(document.getElementById("user-search").value);
        renderStats();
      }));
      tbody.querySelectorAll("[data-delete]").forEach(btn => btn.addEventListener("click", () => {
        ...
        toast({ title: "User deleted", message: `@${name}`, type: "error" });
        renderUsers(document.getElementById("user-search").value);
        renderStats();
      }));
    }
    ...
    function renderCourses() {
      ...
      tbody.innerHTML = courses.map(c => {
        ...
        return `<tr>
          ...
          <td>${c.vip ? `<span class="badge vip">VIP</span>` : `<span class="badge">Free</span>`}</td>
          <td>${c.lessons.length}</td>
          <td>${lockedByAdmin ? `<span class="badge">🔒 Locked</span>` : `<span class="badge done">Available</span>`}</td>
          <td>
            <button class="btn sm ${lockedByAdmin ? "success" : "danger"}" data-lock="${escapeHtml(c.id)}">${lockedByAdmin ? "Unlock" : "Lock"}</button>
          </td>
        </tr>`;
      }).join("");
      tbody.querySelectorAll("[data-lock]").forEach(btn => btn.addEventListener("click", () => {
        ...
        toast({ title: newState ? "Course locked" : "Course unlocked", message: title, type: newState ? "info" : "success" });
        renderCourses();
        renderStats();
      }));
    }
        ...
        <div class="card" style="max-width:820px; width:100%; max-height:85vh; overflow:auto;">
          <div class="row between">
            <div>
              ...
              <div class="small text-muted">${user.vip ? "👑 VIP member" : "Free plan"} · Joined ${new Date(user.createdAt).toLocaleDateString()}</div>
            </div>
            <button class="icon-btn" id="close-modal" aria-label="Close">×</button>
          </div>
          <div class="table-wrap mt-2">
            <table class="table">
              <thead><tr><th>Course</th><th>Lessons</th><th>Progress</th><th>Quizzes</th></tr></thead>
              <tbody>${rows}</tbody>
            </table>
          </div>
        </div>
